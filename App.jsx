...
// --- PERSISTENCE START ---
// We check localStorage during initialization so the data loads immediately.
const [onboarding, setOnboarding] = useState(() => {
return localStorage.getItem('jugGarden_onboarded') === null;
});

const [myZone, setMyZone] = useState(() => {
const saved = localStorage.getItem('jugGarden_myZone');
return saved ? JSON.parse(saved) : null;
});

const [plants, setPlants] = useState(() => {
const saved = localStorage.getItem('jugGarden_plants');
...
// --- AUTO-SAVE EFFECTS ---
useEffect(() => {
localStorage.setItem('jugGarden_plants', JSON.stringify(plants));
}, [plants]);

useEffect(() => {
localStorage.setItem('jugGarden_myZone', JSON.stringify(myZone));
}, [myZone]);

useEffect(() => {
if (!onboarding) {
localStorage.setItem('jugGarden_onboarded', 'true');
}
}, [onboarding]);
// --- AUTO-SAVE EFFECTS END ---
...
{/* ADD PLANT SHEET */}
      {showAdd && (
        ...
        >
          <div 
            style={{ background:"#fff", borderRadius:"22px 22px 0 0", padding:18, width:"100%", maxWidth:480, paddingBottom:34, maxHeight:"86vh", overflowY:"auto" }} 
...
<div style={{ marginBottom: 12 }}>
            ...
            <button onClick={addPlant} style={{ ...btn("linear-gradient(135deg,#43a047,#66bb6a)"), width:"100%", padding:14, fontSize:16 }}>
              Add Plant
            </button>
          </div>
        </div>
...
<button
onClick={addPlant}
style={{ ...btn("linear-gradient(135deg,#43a047,#66bb6a)"), width:"100%", padding:14, fontSize:14, marginTop:10 }}
>
🌱 Add to My Garden
</button>
